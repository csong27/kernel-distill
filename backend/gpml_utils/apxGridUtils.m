function varargout = apxGridUtils(cov, xg, hyp, x, z, b)
...
elseif strcmp(cov,'interp')        % call [Mx,dMx] = apxGrid('interp',xg,x,deg);
  [row, col, val, dval, N]=interpgrid(xg,hyp,x);
  varargout{1}=row; varargout{2}=col; varargout{3}=val;
  varargout{4}=dval; varargout{5}=N; return
...
function [row, col, val, dval, N] = interpgrid(xg,x,deg)
  xg = flattengrid(xg);                                      % remove nestedness
  ...
  if ix
	...
	row = I(id); col = J(id); val = C(id); dval=cell(p,1); 
	for i=1:p, dval{i}=dC{i}(id); end
  end
